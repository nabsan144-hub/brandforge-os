...
# Exact third-party URLs that are allowed to match a staging pattern because
# they are NOT this product's staging hosts: the founder's personal design
# portfolio (his own Vercel deployment), linked from the About section.
ALLOW_EXACT=('https://nabeel-ali-design.vercel.app/')
fail=0

while IFS= read -r file; do
  body=$(cat "$file")
  for ok in "${ALLOW_EXACT[@]}"; do body="${body//$ok/}"; done
