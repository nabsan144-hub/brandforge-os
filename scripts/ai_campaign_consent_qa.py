...
OUT = Path(os.environ.get('BRANDFORGE_QA_DIR', str(ROOT / 'qa-results/ai-campaign-consent')))
...
ORIGIN = 'https://ai-campaign-consent.example.test'
  ...
  for plan in ['free', 'pro']:
   for theme in ['dark', 'light']:
    for width in [390, 1440]:
     errors, submissions = [], []
     campaign = {**FIXTURE['campaign'], 'visual_status': {'mode':'ai', 'state':'generated', 'provider':'openai', 'model':'gpt-image-2.5-sunburst', 'scope':'campaign_formats','formats':3}}
     async def route(r):
      ...
      if path=='/api/config':
       data={'supabaseUrl':ORIGIN, 'supabaseAnonKey':'fixture', 'adSizes':FIXTURE['sizes'], 'images':{'enabled':True, 'provider':'openai', 'model':'gpt-image-2.5-sunburst','campaignEnabled':True,'referenceImages':True}, 'whatsappEnabled':False}
      elif path=='/api/me':
     ...
     if plan=='pro':
      ...
      assert 'Three images' in await page.locator('#image-consent-help').inner_text()
      await box.check()
     ...
     if plan=='pro':
      assert submissions[-1]['artwork_mode']=='campaign'
      assert submissions[-1]['visual_model']=='gpt-image-2.5-sunburst'
      assert submissions[-1]['custom_sizes']==[]
      assert not submissions[-1]['reference_consent']
     assert 'Image: generated' in await page.locator('#d-stage-status').inner_text()
     assert 'Three AI-composed formats' in await page.locator('#d-stage-status').inner_text()
